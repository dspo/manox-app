...
/// shell gutter (2×10) + card borders (2) + sidebar (260) + a readable
/// conversation column (~578). The context rail is a flex sibling of the
/// conversation column (not an overlay), and folds into a drawer below
/// `RAIL_NARROW_BREAK` in agent-ui, so it never constrains the minimum
/// window width.
